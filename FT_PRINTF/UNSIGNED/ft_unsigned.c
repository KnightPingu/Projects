...
int	ft_unsigned(unsigned long long n, char *str, va_list *ap)
{
	...
	if (ft_contains(flags, 'u'))
		return (ft_uint((unsigned int)n, flags, min, max));
	else if (ft_contains(flags, 'x'))
		return (ft_hexlower((unsigned int)n, flags, min, max));
	else if (ft_contains(flags, 'X'))
		return (ft_hexupper((unsigned int)n, flags, min, max));
	else if (ft_contains(flags, 'p'))
		return (ft_address(n, flags, min, max));
	return (-1);
}
